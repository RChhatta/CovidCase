...
st.write('Deze blog gaat over de voortgang van de vaccinatie graad tegen covid. Hiervoor wordt gebruik gemaakt van data van de European Centre for Disease Prevention and Control. In deze blog zal er specifiek gekeken worden naar Nederland en de buurlanden Duitsland en Belgie.')
...
st.write('De data van European Centre for Disease Prevention and Control wordt als een csv file binnengehaald middels een API. Deze wordt dan gelijk gelezen met de pd.read_csv functie. Er zijn twee datasets binnengehaald. Eén dataset gaat over de vaccinaties in Europa en de andere dataset gaat over aantal besmettingen in Europa.')
...
st.write('We zijn dan begonnen met het verkennen van de datasets. De kolommen ‘Region’ en ‘FirstDoseRefused’ zijn verwijderd, omdat deze niet veel informatie toevoegen. ‘FirstDoseRefused’ bevat grotendeels missing values.')
...
st.write('Dan is er ook gekeken naar het aantal percentages gevaccineerde van de totale bevolking per land. Deze zijn in de kolommen ‘PercentageFirstDose’, ‘TotalSecondDose’ en ‘PercentageAdditional1’ gezet.')
...
st.write('We willen ook kijken naar aantal besmettingen per week in een land. Hiervoor wordt de data uit de tweede dataset gehaald. Het land en de datum worden gebruikt als keys om de twee datasets samen te voegen. ')
...
st.write('Voor alledrie landen is Pfizer het meest toegediende vaccin soort. Hierna volgen Moderna en Astrazeneca.')
...
st.write('In deze histogram wordt er gekeken wat de samenstelling van de toegediende vaccinaties zijn verspreid over de afgelopen 2 jaar. (kan worden aangepast met de dropdowns aan de linker zijde). Met de slider kan er geswitcht worden tussen de eerste, tweede en de booster vaccinatie.')
...
st.write('Het aantal besmettingen verspreid over de afgelopen twee jaar wordt in de plot weergeven (kan worden aangepast met de dropdowns aan de linker zijde).')
...
slider = st.select_slider('percentage gevacineerd', options = vergelijking )
